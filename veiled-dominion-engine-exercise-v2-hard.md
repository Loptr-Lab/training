...
**Submission:** fork this repo, implement `engine.ts`, open a PR using `PR_BODY.md` as your description template
...
**Pass bar:** 3.5/5 weighted average, no criterion below 2/5, reaction-framework score below 3 is a hard no.
...
Candidates must not edit `engine.test.ts` or `edge-rules.test.ts`; if they disagree with a test's interpretation of a rule, that goes in `NOTES.md`, not a silent edit. All suites must pass for a submission to be eligible for anything above the minimum Correctness score.

The harness has been verified before being sent to candidates: all tests fail cleanly (not via crash) against the unimplemented stub, and all pass against a working reference implementation.
