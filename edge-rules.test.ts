...
describe('Mandatory Edge Rules — Anti-Cheat & Correctness', () => {
  ...
  describe('Burning Expiry Timing', () => {
    ...
    it('must REMOVE the Burning status when advancing to T+3', () => {
      engine.movePiece({ row: 1, col: 1 }, { row: 1, col: 3 }); // Applies Burning during T

      engine.endTurn(); // Advance to T+1
      engine.endTurn(); // Advance to T+2

      let targetPiece = engine.getPieceAt({ row: 2, col: 3 });
      expect(targetPiece?.status?.type).toBe('Burning');

      engine.endTurn(); // Advance to T+3
      targetPiece = engine.getPieceAt({ row: 2, col: 3 });
